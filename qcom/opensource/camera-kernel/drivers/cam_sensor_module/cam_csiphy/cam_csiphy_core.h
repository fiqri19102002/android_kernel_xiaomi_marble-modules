...
#define XM_MIPI_KMD_CTRL_NO \
	((xm_mipi_kmd_setting >> 56) & (uint64_t)0xff)
#define XM_MIPI_KMD_GET_CTRL_FLAG(ctl_no) \
	((XM_MIPI_KMD_CTRL_NO >> ctl_no) & (uint64_t)0x01)
#define XM_MIPI_KMD_GET_CTRL_VAL(ctl_no) \
	((xm_mipi_kmd_setting >> ((uint64_t)ctl_no << 2) & (uint64_t)0x0F))
//#define XM_MIPI_KMD_GET_CTRL_FLAG_VAL(ctl_no, flag, val) \
//	do {												   \
//		char *p1 = flag;								   \
...
#define XM_MIPI_KMD_CLEAR_CTRL_VAL(ctl_no) \
	(xm_mipi_kmd_setting &= (~((uint64_t)0x0F << (ctl_no << 2))))
#define XM_MIPI_KMD_SET_CTRL_FLAG(ctl_no) \
	(xm_mipi_kmd_setting |= ((uint64_t)1 << (ctl_no + 56)))
#define XM_MIPI_KMD_SET_CTRL_VAL(ctl_no, val) \
	(xm_mipi_kmd_setting |= (((uint64_t)val & 0x0F) << (ctl_no << 2)))
#define XM_MIPI_KMD_SET_CTRL_FLAG_VAL(ctl_no, val) \
	do {										   \
		XM_MIPI_KMD_CLEAR_CTRL_VAL(ctl_no);		   \
		XM_MIPI_KMD_SET_CTRL_FLAG(ctl_no);		   \
		XM_MIPI_KMD_SET_CTRL_VAL(ctl_no, val);	   \
	} while(0)
